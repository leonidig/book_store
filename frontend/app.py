...
@app.get("/")
def index():
    books = {"books": get(f"{BACKEND_URL}/get_books").json()}
...
def product(product_id):
    ...
    if selected_product:
        return render_template(
            "info.html", book=selected_product, product_id=product_id
        )
    else:
...
def cancel_order(book_name):
    data = {"book_name": book_name, "current_user": current_user.email}
